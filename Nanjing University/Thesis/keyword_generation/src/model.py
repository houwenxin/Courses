# -*- coding: utf-8 -*-
"""
Created on Thu Mar  7 16:50:29 2019
...
import numpy as np
import random
from functions import masked_softmax, TimeDistributedDense, GetMask

class Attention(nn.Module):
    def __init__(self, enc_dim, trg_dim, method='general'):
        """
        Attention type: dot or general (by Luong et al.) or concat (by Bahdanau et al.)
        """
    ...
    def score(self, hiddens, encoder_outputs, encoder_mask=None):
        ...
        '''
        if self.method == 'dot':
            # hidden的维度：(batch, trg_len, trg_hidden_dim) * 
            # encoder_outputs的维度：(batch, src_len, src_hidden_dim).transpose(1, 2) = (batch, src_hidden_dim, src_len)
            # 矩阵乘得到energies的维度：(batch, trg_len, src_len)
    ...
    def forward(self, hidden, encoder_outputs, encoder_mask=None):
        ...
        # *乘以 
        # encoder_outputs (batch_size, src_len, src_hidden_dim) 
        # =等于 
        # (batch_size, trg_len, src_hidden_dim)
        ...
        h_tilde = torch.cat((weighted_context, hidden), 2) # 拼接
...
class Seq2SeqLSTMAttention(nn.Module):
    ...
    def __init__(self, opts):
        # 初始化模型
        super(Seq2SeqLSTMAttention, self).__init__() # 调用nn.Module的初始化函数
        self.vocab_size = opts.vocab_size # 字典大小
        self.emb_dim = opts.word_vec_size # 词向量维度（默认：300）
        self.num_directions = 2 if opts.bidirectional else 1 # 双向或是单向LSTM
        self.src_hidden_dim = opts.rnn_size # LSTM隐层维度（默认：512），下同
        self.trg_hidden_dim = opts.rnn_size 
        self.ctx_hidden_dim = opts.rnn_size # ctx: context，上下文
        self.batch_size = opts.batch_size # batch的大小，默认：128
        self.bidirectional = opts.bidirectional # 是否双向
        self.nlayers_src = opts.enc_layers # encoder中的网络层数，默认：1
        self.nlayers_trg = opts.dec_layers # decoder中的网络层数，默认：1
        self.dropout = opts.dropout # dropout率，默认：0

        self.pad_token_src = opts.word2id[vocab.PAD] # PAD和UNK的符号设置
        self.pad_token_trg = opts.word2id[vocab.PAD]
        self.unk_word = opts.word2id[vocab.UNK]

        self.attention_mode = opts.attention_mode # attention的计算方式（三种，默认：general）：'dot', 'general', 'concat'
        self.input_feeding = opts.input_feeding #

        self.copy_attention = opts.copy_attention    # bool, enable copy attention or not
        self.copy_mode = opts.copy_mode         # same to attention_mode`
        self.copy_input_feeding = opts.copy_input_feeding
        self.reuse_copy_attn = opts.reuse_copy_attn # 即Copy的Attention是单独训练还是直接借用普通Attention Layer

        self.must_teacher_forcing = opts.must_teacher_forcing
        self.teacher_forcing_ratio = opts.teacher_forcing_ratio
        self.scheduled_sampling = opts.scheduled_sampling
        self.scheduled_sampling_batches = opts.scheduled_sampling_batches
        ...
        if self.scheduled_sampling:
            print("Applying scheduled sampling with %s decay for the first %d batches" 
        ...
        if self.must_teacher_forcing or self.teacher_forcing_ratio >= 1:
            ...
            print("Training with Teacher Forcing with static rate=%f" % self.teacher_forcing_ratio)
        ...
        if self.copy_attention:
            ...
            print("Applying Copy Mechanism, type=%s" % self.copy_mode)
            # for Gu's model
        ...
        # setup for input-feeding, add a bridge to compress the additional inputs. 
        # Note that input-feeding cannot work with teacher-forcing
        self.dec_input_dim = self.emb_dim  # only input the previous word
        if self.input_feeding:
            print("Applying input feeding")
            self.dec_input_dim += self.trg_hidden_dim
            
        if self.copy_input_feeding:
            print("Applying copy input feeding")
            self.dec_input_dim += self.trg_hidden_dim
    ...
    def init_decoder_state(self, enc_h, enc_c):
        '''
        用于将encoder的输出隐状态转化为适合decoder size的状态
        enc_h, enc_c： (batch_size, src_hidden_size * num_directions)
        decoder_init_hidden, decoder_init_cell:  (1, batch_size, trg_hidden_size)
        unsqueeze即在0维度上增加一维，由(batch_size, trg_hidden_size)->(1, batch_size, trg_hidden_size)
        '''
    ...
    def forward(self, input_src, input_src_len, input_trg, input_src_ext, oov_lists, trg_mask=None, ctx_mask=None):
        '''
        The differences of copy model from normal q here are:
         1. The size of decoder_logits is (batch_size, trg_seq_len, vocab_size + max_oov_number).
            Usually vocab_size=50000 and max_oov_number=1000. 
            And only very few of (it's very rare to have many unk words, in most cases it's because the text is not in English)
        ...
        # src_h_t, src_c_t的size为：(batch_size, hidden_size * num_directions)
        # src_h的size为：(batch, seq_len, num_directions * hidden_size)
    ...
    def encode(self, input_src, input_src_len):
        ...
        # 然后对输入的文章进行词嵌入
        # 初始输入的Size为：(batch_size, src_len)
        # 输出词向量的Size为：(batch_size, src_len, emb_dim)
        src_emb = self.embedding(input_src)
        # 双向RNN必须要用下面这个函数，不然无法计算序列长度，这个函数要和pad_packed_sequence配合使用
        src_emb = nn.utils.rnn.pack_padded_sequence(src_emb, input_src_len, batch_first=True)

        # 将词向量送入编码器
        # 输入的h_0和c_0的size都是：(num_layers * num_directions, batch, hidden_size)
        # 输出的output（即src_h）的size为：(seq_len, batch, num_directions * hidden_size)————即RNN每个timestep最顶层的layer输出的状态
        # 注意这里batch_first=True，所以output的实际size为：(batch, seq_len, num_directions * hidden_size)
        # 注意batch_first只对output有效
        # 输出的h_n和c_n（即src_h_t和src_c_t）的size为：(num_layers * num_directions, batch, hidden_size)————即RNN在最后一个timestep所有layer输出的状态
        ...
        # 如果是双向的，需要把最终时刻的输出拼接起来，拼接后的size为：(batch_size, hidden_size * num_directions)
        ...
        # src_h的维度：(batch_size, seq_len, num_directions * hidden_size)
        # h_t和c_t的维度：(batch_size, hidden_size * num_directions)
    ...
    def merge_decode_inputs(self, trg_emb, h_tilde, copy_h_tilde):
        ...
        if self.dec_input_bridge: # 将维度映射回dec_hidden_size
    ...
    def decode(self, trg_inputs, src_map, oov_list, enc_context, enc_hidden, trg_mask, ctx_mask):
        '''
        :param
                trg_inputs:         (batch_size, trg_len)
                src_map  :          (batch_size, src_len), 
                                    almost the same with src but oov words are replaced with temporary oov index, 
                ...
                enc_hidden：        (batch_size, hidden_size * num_directions)
                                    Tuple: (h_t, c_t)
                enc_context：       (batch_size, seq_len, num_directions * hidden_size)
                ctx_mask:           (batch_size, src_len)
        ...
        context_dim = enc_context.size(2) # 即 num_directions * hidden_size
        trg_hidden_dim = self.trg_hidden_dim
        
        # 将encoder输出的hidden state转化为可以放入decoder size的状态
        # init_decoder_state(self, enc_h, enc_c):
        # enc_h, enc_c： (batch_size, src_hidden_size * num_directions)
        # init_hidden (tuple):  2 * (1, batch_size, trg_hidden_size)
        ...
        if self.attention_layer.method == 'dot':
            enc_context = nn.Tanh() \
                    (self.encoder2decoder_hidden(enc_context.contiguous().view(-1, context_dim))).view(batch_size, src_len, trg_hidden_dim)
            enc_context = enc_context * ctx_mask.view(ctx_mask.size() + (1,))

        # maximum length to unroll, ignore the last word (must be padding)
        max_length = trg_len - 1
        ...
        if not do_word_wisely_training:
            ...
            # 把最后一个单词去掉，因为它一定是PAD，并且后面没有预测的必要
            ...
            # 注意：decoder的batch_first = False；并且decoder只有一个方向
            # decoder_outputs的维度：(trg_len, batch_size, num_directions * hidden_size)
            # dec_hidden (h_t, c_t)的维度：(2, num_layers * num_directions, batch_size, hidden_size)
            ...
            # decoder_outputs.permute(1, 0, 2)的维度：(batch_size, trg_len, trg_hidden_dim)
            # enc_context的维度：(batch_size, src_len, trg_hidden_dim)
            h_tildes, attn_weights, attn_logits = self.attention_layer(decoder_outputs.permute(1, 0, 2), 
                                                                       enc_context, 
            ...
            if self.copy_attention:
                ...
                if not self.reuse_copy_attn:
                    _, copy_weights, copy_logits = self.copy_attention_layer(decoder_outputs.permute(1, 0, 2), 
                                                                             enc_context, 
                                                                             encoder_mask=ctx_mask)
                else:
    ...
    def generate(self, trg_input, dec_hidden, enc_context, ctx_mask=None, src_map=None, oov_list=None, max_len=1, return_attention=False):
        ...
        for i in range(max_len):
            ...
            if not self.copy_attention:
                ...
                # copy_weights and copy_logits is (batch_size, trg_len, src_len)
                if not self.reuse_copy_attn:
                    copy_h_tilde, copy_weight, copy_logit = self.copy_attention_layer(decoder_output.permute(1, 0, 2), 
                                                                                      enc_context, 
                                                                                      encoder_mask=ctx_mask)
                else:
                    copy_h_tilde, copy_weight, copy_logit = h_tilde, attn_weight, attn_logit
...
def init_model(opts, logger):
    logger.info('======================  Initialized Model Parameters  =========================')
    if opts.cascading_model:
        model = Seq2SeqLSTMAttentionCascading(opts)
    else:
        if opts.copy_attention:
            logger.info('Train a Seq2Seq model with copy mechanism')
        else:
            logger.info('Train a normal Seq2Seq model without copy mechanism')
        model = Seq2SeqLSTMAttention(opts)
    
    if opts.train_from: # 注意：train_from是一个path，指向checkpoint的模型文件
        logger.info("Loading previous checkpoint from %s" % opts.train_from)
        if torch.cuda.is_available():
            checkpoint = torch.load(open(opts.train_from, 'rb'))
        else:
            checkpoint = torch.load(open(opts.train_from, 'rb'), map_location=lambda storage, loc: storage)
        ...
        logger.info("Saving initialized model to: %s" % os.path.join(opts.model_path, 'initial.model'))
        # 保存初始模型
        torch.save(
            model.state_dict(),
            open(os.path.join(opts.model_path, 'initial.model'), 'wb')
        )

    utils.tally_parameters(model, logger)
    return model