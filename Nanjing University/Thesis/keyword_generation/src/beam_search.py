# -*- coding: utf-8 -*-
"""
Created on Sat Mar  9 12:24:28 2019
...
class TopN_heap(object):
    ...
    def push(self, x):
        ...
        if len(self._data) < self._n:
            # 关于heapq的用法和堆（heap）：https://www.jianshu.com/p/801318c77ab5
            heapq.heappush(self._data, x)
        else:
            # heapq.heappushpop()：将值插入到堆中同时弹出堆中的最小值。 
...
class SequenceGenerator(object):
    ...
    def sequence_to_batch(self, sequence_lists):
        ...
        :return:
        '''
        seq_id2batch_id = [[sequence.batch_id for sequence in sequence_list.extract()] for sequence_list in sequence_lists]
        ...
        for sequence_list in sequence_lists:
            seq_ids = []
            for sequence in sequence_list.extract():
                seq_ids.append(seq_id)
                seq_id += 1
            flattened_id_map.append(seq_ids)

        # sequence_list: [seq1, seq2, ...]
        # [seq_list1, seq_list2, ..., seq_list(batch_size)] = [[seq1, seq2, ..], [seq1, seq2, ...]]
        # itertools.chain(*):去掉一层中括号[]-->[seq1, seq2, ..., seq1, seq2, ...]
        flattened_sequences = list(itertools.chain(*[sequence_list.extract() for sequence_list in sequence_lists]))
        batch_size = len(flattened_sequences)

        # if it's oov, replace it with <unk> (batch_size, 1)
        # 在0维度上将每句话的最后一个单词拼起来？
        inputs = torch.cat([Variable(torch.LongTensor([sequence.sentence[-1]] 
                    if sequence.sentence[-1] < self.model.vocab_size else [self.model.unk_word])) 
                        for sequence in flattened_sequences]).view(batch_size, -1)
        
        # tuple: (h_t, c_t) / Size：(2 * 1 (tuple), batch_size, trg_hidden_size)
        if isinstance(flattened_sequences[0].dec_hidden, tuple):
            h_states = torch.cat([sequence.dec_hidden[0] for sequence in flattened_sequences]).view(1, batch_size, -1)
            c_states = torch.cat([sequence.dec_hidden[1] for sequence in flattened_sequences]).view(1, batch_size, -1)
            dec_hiddens = (h_states, c_states)

        # TIP: 函数在调用多个参数时，在列表、元组、集合、字典及其他可迭代对象作为实参，并在前面加 *
        contexts = torch.cat([sequence.context for sequence in flattened_sequences]).view(batch_size, *flattened_sequences[0].context.size())
        ctx_mask = torch.cat([sequence.ctx_mask for sequence in flattened_sequences]).view(batch_size, *flattened_sequences[0].ctx_mask.size())
        src_oovs = torch.cat([sequence.src_oov for sequence in flattened_sequences]).view(batch_size, *flattened_sequences[0].src_oov.size())
        oov_lists = [sequence.oov_list for sequence in flattened_sequences]
    ...
    def beam_search(self, src_lists, src_lens, src_oov_lists, oov_lists, word2id):
        ...
        """
        # model.eval()时，PyTorch会自动把 Batch Normalization 和 Dropout 固定住，不会取平均，而是用训练好的值。
        self.model.eval()
        
        batch_size = len(src_lists)

        src_mask = self.get_mask(src_lists)  # same size as src_lists
        """
        model.encode返回：
        :src_context的维度：(batch_size, seq_len, num_directions * hidden_size)
        :src_h和src_t的维度：(batch_size, hidden_size * num_directions)
        """
        src_context, (src_h, src_c) = self.model.encode(src_lists, src_lens)

        # dec_hiddens的Size：2 * (1, batch_size, trg_hidden_size)
        dec_hiddens = self.model.init_decoder_state(src_h, src_c)

        # each dec_hidden is (trg_seq_len, dec_hidden_dim)
        initial_input = [word2id[BOS]] * batch_size
        ...
        # 数据结构：TopN_heap，可以容下的sequence数量最大值不超过给定的参数N的大小。
        ...
        for batch_i in range(batch_size):
            sequence = Sequence(
                batch_id=batch_i, # 从0开始标注batch序号
                sentence=[initial_input[batch_i]],
                dec_hidden=dec_hiddens[batch_i],
                context=src_context[batch_i],
                ctx_mask=src_mask[batch_i],
                src_oov=src_oov_lists[batch_i],
                oov_list=oov_lists[batch_i],
                logprobs=[],
                score=0.0,
                attention=[])
            partial_sequences[batch_i].push(sequence)
            # 现在partial_sequences[batch_i]中self._data为：[sequence]
        ...
        for current_len in range(1, self.max_sequence_length + 1):
            ...
            for batch_i in range(batch_size):
                ...
                logging.info('Batch=%d, \t#(hypothese) = %d, \t#(completed) = %d \t #(new_hyp_explored)=%d' % (batch_i, len(partial_sequences[batch_i]), len(complete_sequences[batch_i]), num_new_hyp_in_batch))
                '''
                # print-out for debug
