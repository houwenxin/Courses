...
public class RelationA implements WritableComparable<RelationA>{
	...
	public boolean isCondition(int col, String value){
		if(col == 0 && Integer.parseInt(value) == this.id)
			...
			judger = value.substring(0, 1);
			ifAge = value.substring(1);
			System.out.println(judger + " " + ifAge);
			if(judger.equals("s") && Integer.parseInt(ifAge) > this.age)
				return true;
			else if(judger.equals("e") && Integer.parseInt(ifAge) == this.age)
				return true;
			else if(judger.equals("l") && Integer.parseInt(ifAge) < this.age)
				return true;
			else
				return false;
		}
