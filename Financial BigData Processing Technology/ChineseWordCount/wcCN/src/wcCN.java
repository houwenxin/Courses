...
public class wcCN {
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (otherArgs.length != 3) {
      System.err.println("Usage: wcCN <in> <out> <k>");
      System.exit(2);
    }
    Path tempDir = new Path("wcCN-temp-" + Integer.toString(new Random().nextInt(Integer.MAX_VALUE)));
    conf.setInt("k", Integer.parseInt(otherArgs[2]));
    
    Job job = new Job(conf, "word count");
    job.setJarByClass(wcCN.class);
    try{
    ...
    if(job.waitForCompletion(true)){
    	...
    	sortJob.setJarByClass(wcCN.class);
